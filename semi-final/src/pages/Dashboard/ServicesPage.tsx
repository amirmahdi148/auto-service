...
export const ServicesPage = () => {
    ...
    const { data: services = [] } = useQuery({
        ...
        queryFn: () => HttpService.get<ServiceItem[]>("/api/dashboard/services"),
    });

    const filteredServices = useMemo(() => {
        if (!search.trim()) return services;
        return services.filter(s => s.name.includes(search.trim()));
    }, [services, search]);

    const createMutation = useMutation({
        mutationFn: (data: { name: string; duration: string; basePrice: number; status: string }) =>
            HttpService.post("/api/dashboard/services", { data }),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["dashboard-services"] }),
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: string; data: { name: string; duration: string; basePrice: number; status: string } }) =>
            HttpService.put(`/api/dashboard/services/${id}`, { data }),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["dashboard-services"] }),
    });

    const deleteMutation = useMutation({
        mutationFn: (id: string) => HttpService.delete(`/api/dashboard/services/${id}`),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["dashboard-services"] }),
    });
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        const payload = { name: formData.name, duration: formData.duration, basePrice: price, status: formData.status };
