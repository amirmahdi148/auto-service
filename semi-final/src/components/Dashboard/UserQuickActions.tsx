...
export const UserQuickActions = ({ onNewBooking }: UserQuickActionsProps) => {
    const navigate = useNavigate();
    const USER_QUICK_ACTIONS = [
        { Icon: CalendarPlus, label: "رزرو جدید", onClick: () => onNewBooking?.() },
        { Icon: Wrench, label: "خدمات ما", onClick: () => navigate("/services") },
        { Icon: MapPinned, label: "مراکز نزدیک", onClick: () => navigate("/dashboard/favorites") },
        { Icon: BadgePercent, label: "تخفیف‌ها", onClick: () => navigate("/services") },
        { Icon: MessageSquareHeart, label: "پشتیبانی", onClick: () => navigate("/dashboard/support") },
        { Icon: ShieldAlert, label: "امداد خودرو", onClick: () => window.open("https://112.ir", "_blank") },
    ];
